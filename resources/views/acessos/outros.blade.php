<div class="table-responsive" style="margin: 0 !important;">
    <table class="table table-hover">
        ...
        <tbody>
            @forelse($outrosCertificados as $obj)
                <tr id="row_{{ $obj[0]->id }}" class="@if ($obj[0]->deleted_at != null) table-danger @endif">
                    <td>{{ App\Helpers\FormatterHelper::formatCnpjCpf($obj[0]->chave) }}</td>
                    ...
                    <td>
                        <form action="{{ route('acessos.status', [$obj[0]->id]) }}" method="GET" id="form_{{ $obj[0]->id }}"
                            ...
                            <button type="button" style="border: none; background: none; box-shadow: none;" class="btn btn-sm btn-secondary" onclick="Tela.abrirJanela('{{ route('acessos.opcoes', [$obj[0]->id]) }}', 'Atualizar Acesso', 'xs')">
